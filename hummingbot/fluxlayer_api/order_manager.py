...
class OrderManager:
        ...
        for i in range(15):  # 最多等待30秒
            try:
                if trading_pair in ob_tracker.order_books:
                    ...
                    bid_count = len(list(order_book.bid_entries()))
                    ask_count = len(list(order_book.ask_entries()))
    ...
    async def _fix_missing_order_book_tracker(self, connector: Any, connector_name: str):
        """
        修复缺失的订单簿跟踪器
        ...
        self._logger.info(f"🔧 [BYBIT FIX] Attempting to fix missing order book tracker...")

        try:
            # 检查是否真的缺失
            if hasattr(connector, '_order_book_tracker') and connector._order_book_tracker:
                self._logger.info(f"✅ [BYBIT FIX] Order book tracker already exists")
                return

            # 方法1：尝试重新创建订单簿跟踪器
            if hasattr(connector, '_create_order_book_data_source'):
                try:
                    self._logger.info(f"🔧 [BYBIT FIX] Method 1: Creating order book tracker from scratch...")

                    # 创建订单簿数据源
                    orderbook_ds = connector._create_order_book_data_source()
                    self._logger.info(f"🔧 [BYBIT FIX] Created order book data source: {type(orderbook_ds).__name__}")

                    # 创建订单簿跟踪器
                    from hummingbot.core.data_type.order_book_tracker import OrderBookTracker

                    tracker = OrderBookTracker(
                        data_source=orderbook_ds,
                        trading_pairs=connector._trading_pairs or ["BTC-USDT"],
                        domain=connector.domain if hasattr(connector, 'domain') else "bybit_main"
                    )
                    self._logger.info(f"🔧 [BYBIT FIX] Created order book tracker")

                    # 设置订单簿跟踪器
                    if hasattr(connector, '_set_order_book_tracker'):
                        connector._set_order_book_tracker(tracker)
                        self._logger.info(f"✅ [BYBIT FIX] Set order book tracker via _set_order_book_tracker")
                    else:
                        # 直接设置属性
                        connector._order_book_tracker = tracker
                        self._logger.info(f"✅ [BYBIT FIX] Set order book tracker directly")

                    # 启动订单簿跟踪器
                    if hasattr(tracker, 'start'):
                        tracker.start()
                        self._logger.info(f"✅ [BYBIT FIX] Started order book tracker")
                    ...
                    if hasattr(connector, '_order_book_tracker') and connector._order_book_tracker:
                        ...
                        self._logger.info(f"✅ [BYBIT FIX] Order book tracker verification: {order_books_count} order books")

                        if order_books_count == 0:
                            # 强制触发订单簿数据加载
                            self._logger.info(f"🔧 [BYBIT FIX] Forcing order book data loading...")
                            if hasattr(ob_tracker, '_data_source') and hasattr(ob_tracker._data_source, 'start'):
                                ob_tracker._data_source.start()
                                await asyncio.sleep(5)

                                # 再次检查
                                order_books_count = len(ob_tracker.order_books)
                                self._logger.info(f"🔧 [BYBIT FIX] After forced loading: {order_books_count} order books")

                        return
                    else:
                        self._logger.error(f"❌ [BYBIT FIX] Order book tracker still not set after creation")

                except Exception as method1_error:
                    self._logger.error(f"❌ [BYBIT FIX] Method 1 failed: {method1_error}")

            # 方法2：尝试重新初始化整个网络组件
            self._logger.info(f"🔧 [BYBIT FIX] Method 2: Attempting network reinitialization...")
            try:
                if hasattr(connector, 'start_network'):
                    # 停止现有网络（如果有）
                    ...
                    # 检查结果
                    if hasattr(connector, '_order_book_tracker') and connector._order_book_tracker:
                        self._logger.info(f"✅ [BYBIT FIX] Method 2 successful: order book tracker restored")
                        return

            except Exception as method2_error:
                self._logger.error(f"❌ [BYBIT FIX] Method 2 failed: {method2_error}")

            # 方法3：创建最小化的订单簿跟踪器（仅用于价格获取）
            self._logger.info(f"🔧 [BYBIT FIX] Method 3: Creating minimal order book tracker...")
            try:
                from hummingbot.core.data_type.order_book_tracker import OrderBookTracker
                from hummingbot.core.data_type.order_book import OrderBook

                # 创建空的订单簿跟踪器
                class DummyOrderBookDataSource:
                    def __init__(self):
                        pass
                ...
                dummy_ds = DummyOrderBookDataSource()
                tracker = OrderBookTracker(
                ...
                try:
                    # 尝试从API获取当前价格
                    if hasattr(connector, '_api_get'):
                        ticker_response = await connector._api_get(
                            path_url='/v5/market/tickers',
                            params={'category': 'spot', 'symbol': 'BTCUSDT'}
                        )
                        if ticker_response and 'result' in ticker_response:
                            last_price = float(ticker_response['result']['list'][0]['lastPrice'])

                            # 使用正确的OrderBook方法添加买卖档位
                            import numpy as np

                            # 创建简单的买卖档位数据
                            bids_data = np.array([[last_price * 0.999, 1.0]], dtype=np.float64)
                            asks_data = np.array([[last_price * 1.001, 1.0]], dtype=np.float64)

                            # 使用numpy格式应用快照
                            order_book.apply_numpy_snapshot(bids_data, asks_data, 1)

                            self._logger.info(f"🔧 [BYBIT FIX] Added price data to dummy order book: {last_price}")
                except Exception as price_error:
                    self._logger.warning(f"⚠️ [BYBIT FIX] Could not get price data: {price_error}")
                    # 使用默认价格创建订单簿快照
                    try:
                        import numpy as np
                        bids_data = np.array([[99000.0, 1.0]], dtype=np.float64)
                        asks_data = np.array([[100000.0, 1.0]], dtype=np.float64)
                        order_book.apply_numpy_snapshot(bids_data, asks_data, 1)
                        self._logger.info(f"🔧 [BYBIT FIX] Used default price data for dummy order book")
                    except Exception as default_error:
                        self._logger.warning(f"⚠️ [BYBIT FIX] Could not set default price data: {default_error}")

                tracker.order_books["BTC-USDT"] = order_book

                # 设置订单簿跟踪器
                connector._order_book_tracker = tracker
                self._logger.info(f"✅ [BYBIT FIX] Method 3 successful: minimal order book tracker created")

            except Exception as method3_error:
                self._logger.error(f"❌ [BYBIT FIX] Method 3 failed: {method3_error}")

        except Exception as fix_error:
            self._logger.error(f"❌ [BYBIT FIX] Overall fix attempt failed: {fix_error}")
            import traceback
            self._logger.error(f"❌ [BYBIT FIX] Fix error traceback: {traceback.format_exc()}")
        ...
        async with self._connector_locks[connector_name]:
            ...
            try:
                ...
                if connector_name == "bybit":
                    ...
                    # 修复订单簿跟踪器缺失问题
                    await self._fix_missing_order_book_tracker(connector, connector_name)
                            ...
                            if ob_tracker:
                                ...
                                if trading_pair in ob_tracker.order_books:
                                    ...
                                    try:
                                        ...
                                        self._logger.warning(f"⚠️ [BYBIT DEBUG] Error accessing order book entries: {entries_error}")
                                        bid_count = 0
                                        ask_count = 0
                        ...
                        if hasattr(connector, '_order_book_tracker') and connector._order_book_tracker:
                            ...
                            if trading_pair in ob_tracker.order_books:
                                ...
                                try:
                                    ...
                                    self._logger.warning(f"⚠️ [BYBIT DEBUG] Error accessing order book entries after wait: {entries_error}")
                                    bid_count = 0
                                    ask_count = 0
    ...
    async def _place_order_bybit_direct(
        self,
        trading_pair: str,
        amount: float,
        is_buy: bool,
        ...
        """
        直接调用Bybit API下单，使用类似 test_bybit_api.py 的逻辑
        """
        try:
            # 获取API密钥
            ...
            if not api_key or not api_secret:
                return {
                    "success": False,
                    "error": "Missing BYBIT_API_KEY or BYBIT_API_SECRET environment variables"
                }

            self._logger.info(f"🔍 [BYBIT DIRECT] API Key: {api_key[:8]}...")
            self._logger.info(f"🔍 [BYBIT DIRECT] Using trading pair: {trading_pair}, amount: {amount}")

            # 获取代理设置
            proxy = os.getenv("HTTP_PROXY") or os.getenv("http_proxy")

            # 准备基础参数
            symbol = trading_pair.replace("-", "")  # BTC-USDT -> BTCUSDT
            side = "Buy" if is_buy else "Sell"
            order_type_map = {"MARKET": "Market", "LIMIT": "Limit"}
            bybit_order_type = order_type_map.get(order_type, "Market")

            # 先获取交易规则来调整数量精度
            try:
                self._logger.info(f"🔍 [BYBIT DIRECT] Getting trading rules for {symbol}")
                instruments_url = "https://api.bybit.com/v5/market/instruments-info"
                instruments_params = {"category": "spot", "symbol": symbol}

                async with aiohttp.ClientSession() as session:
                    async with session.get(instruments_url, params=instruments_params, proxy=proxy) as response:
                        instruments_result = await response.json()

                if instruments_result.get("retCode") == 0:
                    instrument_info = instruments_result["result"]["list"][0]
                    lot_size_filter = instrument_info.get("lotSizeFilter", {})
                    min_order_qty = float(lot_size_filter.get("minOrderQty", "0.000001"))
                    qty_step = float(lot_size_filter.get("qtyStep", "0.000001"))

                    # 根据步长调整数量
                    adjusted_amount = round(amount / qty_step) * qty_step

                    # 确保不小于最小订单量
                    if adjusted_amount < min_order_qty:
                        adjusted_amount = min_order_qty

                    self._logger.info(f"🔍 [BYBIT DIRECT] Trading rules - min_qty: {min_order_qty}, step: {qty_step}")
                    self._logger.info(f"🔍 [BYBIT DIRECT] Adjusted amount: {amount} -> {adjusted_amount}")
                else:
                    self._logger.warning(f"⚠️ [BYBIT DIRECT] Could not get trading rules, using original amount")
                    adjusted_amount = amount

            except Exception as e:
                self._logger.warning(f"⚠️ [BYBIT DIRECT] Error getting trading rules: {e}, using original amount")
                adjusted_amount = amount

            # 格式化数量 - 使用6位小数精度并去除尾随零
            formatted_qty = f"{adjusted_amount:.6f}".rstrip('0').rstrip('.')
            if not formatted_qty or formatted_qty == '.':
                formatted_qty = f"{adjusted_amount:.6f}"
            ...
            api_params = {
                ...
                "qty": formatted_qty,
                "marketUnit": "baseCoin",
                "orderLinkId": f"DIRECT-{int(time.time())}"
            }

            if order_type == "LIMIT" and price is not None:
                api_params["price"] = f"{price:.2f}"
                api_params["timeInForce"] = "GTC"

            self._logger.info(f"🔍 [BYBIT DIRECT] Order parameters: {json.dumps(api_params, indent=2)}")

            # 生成签名 - 使用与 test_bybit_api.py 相同的逻辑
            timestamp = str(int(time.time() * 1000))
            params_str = json.dumps(api_params)  # 不指定separators，使用默认格式
            param_str = f"{timestamp}{api_key}5000{params_str}"
            signature = hmac.new(
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    self._logger.info(f"🔍 [BYBIT DIRECT] API Response: {json.dumps(result, indent=2)}")

                    if result.get("retCode") == 0:
                        order_result = result.get("result", {})
                        self._logger.info(f"🎉 [BYBIT DIRECT] Order created successfully!")
                        self._logger.info(f"🎉 [BYBIT DIRECT] Order ID: {order_result.get('orderId')}")
                        self._logger.info(f"🎉 [BYBIT DIRECT] Order Link ID: {order_result.get('orderLinkId')}")

                        return {
                            "success": True,
                            "order_id": order_result.get("orderLinkId", ""),
                        ...
                        self._logger.error(f"❌ [BYBIT DIRECT] Order failed (code: {error_code}): {error_msg}")

                        return {
                            "success": False,
                            "error": f"Bybit API error (code: {error_code}): {error_msg}",
            ...
            self._logger.error(f"❌ [BYBIT DIRECT] Error placing order: {e}")
            import traceback
            self._logger.error(f"❌ [BYBIT DIRECT] Traceback: {traceback.format_exc()}")
            return {
                "success": False,
    ...
    async def place_order(
        ...
        **kwargs
        ...
        try:
            ...
            # 如果是 Bybit，直接使用简化的 API 调用逻辑
            if connector_name.lower() == "bybit":
                self._logger.info(f"🔍 [BYBIT DIRECT] Using direct API for Bybit order placement")
                return await self._place_order_bybit_direct(trading_pair, amount, is_buy, order_type, price)
            ...
            try:
                    ...
                    if hasattr(connector, '_order_book_tracker') and connector._order_book_tracker:
                        ...
                        if trading_pair in ob_tracker.order_books:
                            ...
                            try:
                                ...
                                self._logger.warning(f"⚠️ [BYBIT ORDERBOOK] Error accessing order book entries: {entries_error}")
                                bid_count = 0
                                ask_count = 0
                            ...
                            if bid_count == 0 or ask_count == 0:
                                if order_type == "MARKET":
                                    ...
                                    try:
                                        ...
                                        # 再次检查
                                        bid_count_after = len(list(order_book.bid_entries()))
                                        ask_count_after = len(list(order_book.ask_entries()))
                ...
                # 对于Bybit，如果Hummingbot连接器失败，尝试直接API调用
                if connector_name.lower() == "bybit":
                    self._logger.warning(f"⚠️ [BYBIT FALLBACK] Hummingbot connector failed: {e}, trying direct API call")
                    return await self._place_order_bybit_direct(trading_pair, amount, is_buy, order_type, price)
                ...
                if ob_tracker:
                    ...
                    if trading_pair in ob_tracker.order_books:
                        ...
                        try:
                            ...
                            self._logger.warning(f"⚠️ [DIAGNOSIS] Error counting order book entries: {ob_error}")
                            target_pair_ready = False
                            target_pair_data = {"error": str(ob_error)}
