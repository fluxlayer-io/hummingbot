...
from hummingbot.connector.exchange.fluxlayer import (
    fluxlayer_constants as CONSTANTS,
    fluxlayer_utils,
    fluxlayer_web_utils as web_utils,
)
from hummingbot.connector.exchange.fluxlayer.fluxlayer_api_order_book_data_source import FluxlayerAPIOrderBookDataSource
from hummingbot.connector.exchange.fluxlayer.fluxlayer_api_user_stream_data_source import (
    FluxlayerAPIUserStreamDataSource,
)
from hummingbot.connector.exchange.fluxlayer.fluxlayer_auth import FluxlayerAuth
...
class FluxlayerExchange(ExchangePyBase):
    ...
    def __init__(self,
                 ...
                 fluxlayer_api_key: str,
                 fluxlayer_api_secret: str,
                 trading_pairs: Optional[List[str]] = None,
                 trading_required: bool = True,
                 domain: str = CONSTANTS.DEFAULT_DOMAIN,
                 ):
        self.api_key = fluxlayer_api_key
        self.secret_key = fluxlayer_api_secret
        self._domain = domain
        self._trading_required = trading_required
        self._trading_pairs = trading_pairs
        self._last_trades_poll_fluxlayer_timestamp = 1.0
        self.metadata = None
        super().__init__(client_config_map)

    @staticmethod
    def fluxlayer_order_type(order_type: OrderType) -> str:
        return order_type.name.upper()

    @staticmethod
    def to_hb_order_type(fluxlayer_type: str) -> OrderType:
        return OrderType[fluxlayer_type]

    @property
    def authenticator(self):
        return FluxlayerAuth(
    ...
    def name(self) -> str:
        if self._domain == "com":
            ...
            return f"fluxlayer_{self._domain}"
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return FluxlayerAPIOrderBookDataSource(
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return FluxlayerAPIUserStreamDataSource(
            auth=self._auth,
            trading_pairs=self._trading_pairs,
            connector=self,
            api_factory=self._web_assistants_factory,
    ...
    async def _place_order(self,
        ...
        type_str = FluxlayerExchange.fluxlayer_order_type(order_type)
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://github.com/fluxlayer-exchange/fluxlayer-official-api-docs/blob/master/user-data-stream.md
                # As per the order update section in Fluxlayer the ID of the order being canceled is under the "C" key
    ...
    async def _update_order_fills_from_trades(self):
        ...
        in case Fluxlayer's user stream events are not working.
        NOTE: It is not required to copy this functionality in other connectors.
        This is separated from _update_order_status which only updates the order status without producing filled
        events, since Fluxlayer's get order endpoint does not return trade IDs.
        ...
        if (long_interval_current_tick > long_interval_last_tick
            ...
            query_time = int(self._last_trades_poll_fluxlayer_timestamp * 1e3)
            self._last_trades_poll_fluxlayer_timestamp = self._time_synchronizer.time()
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(fluxlayer_utils.is_exchange_information_valid, exchange_info["symbols"]):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=params, headers={"Content-Type": "application/json"}) as resp:
                    if resp.status != 200:
                        self.logger().error(f"Quote request failed: HTTP {resp.status}")
                        return None
            ...
            self.logger().error(f"Error getting quote price: {str(e)}", exc_info=True)
            return None
